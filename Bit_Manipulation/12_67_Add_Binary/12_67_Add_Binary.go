...
func addBinary(a string, b string) string {
	...
	// Еще важно уточнить, что возникает баг из-за того, что массив result создаётся длиной maxLen+1,
	// и первый элемент по умолчанию остаётся нулевым байтом (0x00), если при сложении не был использован перенос.
	// Когда потом делаешь string(result), этот нулевой байт попадает в строку → Go печатает его как \u0000
	// Поэтому в проверку добавляем  || result[0] == 0 потому, что '0' = 48
